...
export default function StudioError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div style={{ padding: '4rem 2rem', textAlign: 'center' }}>
      <h2 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '0.5rem' }}>Studio failed to load</h2>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
        ...
        <a href="/dashboard" style={{ padding: '0.5rem 1.25rem', background: 'var(--surface-raised, var(--surface-card))', border: '1px solid var(--border)', borderRadius: '6px', color: 'var(--text-muted)', fontSize: '0.875rem', textDecoration: 'none' }}>
          Dashboard
        </a>
      </div>
    </div>
